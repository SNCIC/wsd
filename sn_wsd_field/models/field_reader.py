...
class SnFieldReader(models.AbstractModel):
    """Model-agnostic field reader shared by the label center and the
    coding-rule engine.

    ``field_path`` dot notation walks many2one hops and must end on a
    whitelisted scalar field type, so any business model can consume it
    without business-module coupling (label center + sn.code.rule both
    build on this).
    """

    _name = 'sn.field.reader'
    _description = 'Field Path Reader'
