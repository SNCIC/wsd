...
class TestTooling(TransactionCase):
    ...
    def test_tooling_sn_generated_by_code_rule(self):
        # mes-coding-rule batch 5: an empty SN is generated by the engine
        # (seeded rule: TG-YYYYMMDD-NNNN, counter reset daily).
        tooling = self.env['sn.tooling'].create({'template_id': self.template.id})
        today = fields.Date.context_today(tooling)
        self.assertRegex(
            tooling.sn, r'^TG-%s-\d{4}$' % today.strftime('%Y%m%d'))

    def test_tooling_sn_manual_value_wins(self):
        tooling = self._create_tooling('TOOL-MANUAL-1')
        self.assertEqual(tooling.sn, 'TOOL-MANUAL-1')

    def test_tooling_sn_generated_unique_per_create(self):
        first = self.env['sn.tooling'].create({'template_id': self.template.id})
        second = self.env['sn.tooling'].create({'template_id': self.template.id})
        self.assertNotEqual(first.sn, second.sn)
        self.assertTrue(re.match(r'^TG-\d{8}-\d{4}$', first.sn))
        self.assertTrue(re.match(r'^TG-\d{8}-\d{4}$', second.sn))
