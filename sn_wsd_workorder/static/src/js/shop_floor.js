...
export class SnWsdShopFloor extends Component {
    ...
    setup() {
        ...
        onWillStart(async () => {
            // reload() also brings the signed-in operator; the station tab
            // is the whole terminal now
            await Promise.all([this.reload(), this.loadStation()]);
        });
    ...
    async switchTab(tab) {
        this.state.tab = tab;
        if (tab === "station" && !this.state.station.workcenters.length) {
