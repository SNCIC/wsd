...
class MrpEcoApprovalTemplate(models.Model):
    ...
    user_ids = fields.Many2many('res.users', string='Users', domain=lambda self: [('all_group_ids', 'in', self.env.ref('sn_wsd_plm.group_plm_user').id)], required=True)
...
class MrpEco(models.Model):
                ...
                if not approval:
                    ...
                    for user in approval_template.user_ids:
                        ...
                        activity_vals.append({
                            ...
                            'user_id': user.id,
                            'res_id': eco.id,
                            'res_model_id': self.env.ref('sn_wsd_plm.model_mrp_eco').id,
                        })
    ...
    def action_see_attachments(self):
        self.ensure_one()
        attachment_view = self.env.ref('sn_wsd_plm.view_document_file_kanban_sn_wsd_plm')
        context = {
...
class MrpEcoBomChange(models.Model):
    ...
    def action_open_component_change(self):
        ...
        return {
            ...
            'view_mode': 'form',
            'view_id': self.env.ref('sn_wsd_plm.mrp_bom_view_form_inherit_plm_components').id,
        }
    ...
    def action_open_byproduct_change(self):
        ...
        return {
            ...
            'view_mode': 'form',
            'view_id': self.env.ref('sn_wsd_plm.mrp_bom_view_form_inherit_plm_byproducts').id,
        }
