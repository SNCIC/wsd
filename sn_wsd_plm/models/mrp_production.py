...
class MrpProduction(models.Model):
    ...
    def action_create_eco(self):
        ...
        eco_type = self.env.ref('sn_wsd_plm.ecotype_bom_update', raise_if_not_found=False)
        if not eco_type:
            eco_type = self.env['mrp.eco.type'].sudo().search([], limit=1)
        eco_stage = eco_type.stage_ids[:1]
        action = self.env['ir.actions.act_window']._for_xml_id('sn_wsd_plm.mrp_eco_action_main')
