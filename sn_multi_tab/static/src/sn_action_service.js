...
export function makeActionManager(env, router = _router) {
    ...
    let id = 0;
    // sn: per-tab controller stacks, keyed by action jsId. Each tab owns an
    // independent stack so that controllers of one tab never interfere with
    // another tab. The single shared `controllerStack` of the standard service
    // is replaced by `_getCurrentControllerStack()` / `_getControllerStack()`.
    let snStacks = {};
    ...
    rpcBus.addEventListener("RPC:RESPONSE", async (ev) => {
        ...
        if (model === "ir.actions.act_window" && UPDATE_METHODS.includes(method)) {
            ...
            _setCurrentControllerStack(nextStack);
        }
    });
    ...
    /**
     * sn: get the controller stack bound to a given action jsId, creating it
     * on first access.
     * @param {string} actionJsId
     * @returns {ControllerStack}
     */
    function _getControllerStack(actionJsId) {
        if (actionJsId) {
            return snStacks[actionJsId] || (snStacks[actionJsId] = []);
        }
        return [];
    }

    /**
     * sn: returns the controller stack of the currently active tab.
     * @returns {ControllerStack}
     */
    ...
    /**
     * sn: replace the stack of the currently active tab.
     * @param {ControllerStack} stack
     */
    function _setCurrentControllerStack(stack) {
        if (currentActionJsId) {
            snStacks[currentActionJsId] = stack;
        }
    }

    /**
     * sn: whether the given controller belongs to the active tab's stack.
     * Used to make inactive (still mounted) tabs inert to global events.
     * @param {Controller} controller
     * @returns {boolean}
     */
    function _isActiveController(controller) {
        return _getCurrentControllerStack().includes(controller);
    }
    ...
    function _getCurrentController() {
        const stack = _getCurrentControllerStack();
     ...
     */
    function _computeStackIndex(options, controllerStack) {
    ...
    async function _updateUI(controller, options = {}) {
        ...
        // sn: resolve the controller stack of the tab this controller belongs
        // to. A tab is identified by its ROOT action's jsId, which stays stable
        // while navigating inside the tab: navigation inside a tab (list -> form,
        // m2o clicks, breadcrumbs, ...) must push onto the active tab's stack
        // instead of creating a new tab, while menu/app navigation
        // (clearBreadcrumbs) starts a brand-new tab.
        const jsId = (controller.action && controller.action.jsId) || null;
        let tabJsId = null;
        let controllerStack;
        if (action.target !== "new" && "newStack" in options) {
            // the restored stack belongs to the tab of the action being opened
            controllerStack = options.newStack;
            tabJsId = jsId;
            snStacks[jsId] = options.newStack;
        } else if (jsId && snStacks[jsId] !== undefined) {
            // the action is already bound to a tab (switchView / restore / re-open
            // of the tab's current action): reuse its stack
            controllerStack = snStacks[jsId];
            tabJsId = jsId;
        } else if (options.clearBreadcrumbs) {
            // menu/app navigation: start a brand-new tab with a fresh stack
            controllerStack = snStacks[jsId] = [];
            tabJsId = jsId;
        } else {
            // navigation inside the current tab: push onto the active tab's stack
            controllerStack = _getCurrentControllerStack();
            tabJsId = currentActionJsId || jsId;
            if (!currentActionJsId && jsId) {
                // no tab exists yet (e.g. the very first action): this action
                // roots the tab
                snStacks[jsId] = controllerStack;
            }
        }
        const index = _computeStackIndex(options, controllerStack);
        ...
        class ControllerComponent extends Component {
            ...
            setup() {
                ...
                if (action.target !== "new") {
                    ...
                    useBus(env.bus, "CLEAR-UNCOMMITTED-CHANGES", (ev) => {
                        // sn: only the active tab may answer a leave/save
                        // request. Inactive (still mounted) tabs stay inert so
                        // that saving one tab never saves the others.
            ...
            onMounted() {
                if (action.target === "new") {
                    ...
                    // sn: only commit the shared/global state when this
                    // controller belongs to the active tab. This prevents an
                    // inactive tab from overwriting the URL state, the session
                    // storage and the committed stack on mount. The commit
                    // below populates the tab's stack, so it must not depend on
                    // the stack contents (which are still empty at mount time).
                if (tabJsId && tabJsId === currentActionJsId) {
                    _setCurrentControllerStack(nextStack); // the controller is mounted, commit the new stack
                    pushState();
        ...
        controller.__info__ = {
            ...
            // sn_multi_tab: expose the action + display name so the tab bar
            // can dedup and label tabs (Odoo 19 no longer puts `action` in props)
            action: controller.action,
            displayName: controller.displayName,
            // sn_multi_tab: identity of the tab this controller belongs to
            // (the jsId of the ROOT action of the tab's stack) and the root
            // action itself, which gives the tab its stable title
            tabJsId,
            rootAction: (nextStack[0] && nextStack[0].action) || controller.action,
        };
    ...
    async function restore(jsId) {
        ...
        if (controller.virtual) {
            ...
            _setCurrentControllerStack(controllerStack);
    ...
    function pushState(cStack = _getCurrentControllerStack(), options) {
        if (!cStack.length) {
            return;
        }
    ...
    return {
        ...
        // sn: multi-tab API consumed by the ActionContainer patch.
        setCurrentAction(jsId) {
            currentActionJsId = jsId;
        },
        // sn: sync the URL and the document title with the stack of the
        // currently active tab. Only called when a tab is activated WITHOUT
        // mounting a controller (tab click / close fallback): a controller
        // mount commits its own state in the onMounted hook, and pushing here
        // as well would merge a stale stack into the debounced URL update.
        syncTabState() {
            const stack = _getCurrentControllerStack();
            if (stack.length) {
                pushState();
                env.services.title.setParts({ action: stack.at(-1).displayName });
            }
        },
        getCurrentActionJsId() {
            return currentActionJsId;
        },
        removeStacks(actionJsIds) {
            for (const jsId of actionJsIds) {
                delete snStacks[jsId];
            }
        },
        removeAllStacks() {
            snStacks = {};
