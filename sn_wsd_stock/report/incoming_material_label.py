...
class ReportIncomingMaterialLabelZpl(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        for lot in lots:
            labels.append({
                'material_code': Markup(self._clean_zpl_text(
                    lot.product_id.default_code
                )),
                'material_name': Markup(self._clean_zpl_text(
                    lot.product_id.name
                )),
                'specification': Markup(self._clean_zpl_text(
                    lot.product_id.material_specification
                )),
