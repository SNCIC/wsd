...
class TestConsumable(TransactionCase):
    ...
    def test_info_sn_generated_by_code_rule(self):
        # mes-coding-rule batch 5: an empty SN is generated by the engine
        # (seeded rule: FL-YYYYMMDD-NNNN, counter reset daily).
        info = self.env['sn.consumable.info'].create({
            'template_id': self.template.id,
        })
        today = fields.Date.context_today(info)
        self.assertRegex(info.sn, r'^FL-%s-\d{4}$' % today.strftime('%Y%m%d'))

    def test_info_sn_manual_value_wins(self):
        info = self.env['sn.consumable.info'].create({
            'sn': 'AUX-MANUAL-1',
            'template_id': self.template.id,
        })
        self.assertEqual(info.sn, 'AUX-MANUAL-1')

    def test_info_sn_generated_unique_per_create(self):
        first = self.env['sn.consumable.info'].create({
            'template_id': self.template.id,
        })
        second = self.env['sn.consumable.info'].create({
            'template_id': self.template.id,
        })
        self.assertNotEqual(first.sn, second.sn)
        self.assertTrue(re.match(r'^FL-\d{8}-\d{4}$', first.sn))
        self.assertTrue(re.match(r'^FL-\d{8}-\d{4}$', second.sn))
