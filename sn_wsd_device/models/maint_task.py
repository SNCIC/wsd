...
class MaintenanceTask(models.Model):
    """Maintenance task generated from a maintenance plan."""
    _name = 'sn.wsd.device.maint.task'
    _description = 'Equipment Maintenance Task'
    ...
    line_ids = fields.One2many(
        'sn.wsd.device.maint.task.line', 'task_id', string='Maintenance Lines')
    ...
    def action_submit(self):
        for task in self:
            ...
            task.equipment_id.last_maintenance_date = fields.Datetime.now()

    @api.model
    def _mark_previous_unfinished_overdue(self):
        """Called by the maintenance generation cron before generating:
        uncompleted tasks from previous days become overdue."""
...
class MaintenanceTaskLine(models.Model):
    """One maintenance item of a task, copied from the plan template."""
    _name = 'sn.wsd.device.maint.task.line'
    _description = 'Equipment Maintenance Task Line'
    _order = 'sequence, id'
    _rec_name = 'name'

    task_id = fields.Many2one(
        'sn.wsd.device.maint.task', string='Maintenance Task',
    ...
    def action_download_guide(self):
        ...
        return {
            ...
            'url': f'/web/content?model=sn.wsd.device.maint.task.line'
