...
class StockQuant(models.Model):
    ...
    def barcode_write(self, vals):
        ...
        return self.browse(quant_ids)._get_sn_wsd_barcode_data()
    ...
    def action_client_action(self):
        ...
        """
        action = self.env['ir.actions.actions']._for_xml_id('sn_wsd_barcode.sn_wsd_barcode_inventory_client_action')
        return action
    ...
    def _get_sn_wsd_barcode_data(self):
        ...
        data = valid_quants.with_context(display_default_code=False, barcode_view=True).get_sn_wsd_barcode_data_records()
        if locations:
            data["records"]["stock.location"] = locations.read(locations._get_fields_sn_wsd_barcode(), load=False)
        if package_types:
            data["records"]["stock.package.type"] = package_types.read(package_types._get_fields_sn_wsd_barcode(), load=False)
        data['line_view_id'] = self.env.ref('sn_wsd_barcode.stock_quant_barcode').id
        return data

    def get_sn_wsd_barcode_data_records(self):
        ...
        data = {
            "records": {
                "stock.quant": self.read(self._get_fields_sn_wsd_barcode(), load=False),
                "product.product": products.read(products._get_fields_sn_wsd_barcode(), load=False),
                "stock.package": packages.read(packages._get_fields_sn_wsd_barcode(), load=False),
                "res.company": companies.read(['name']),
                "res.partner": owners.read(owners._get_fields_sn_wsd_barcode(), load=False),
    ...
    def _get_sn_wsd_barcode_specific_data(self):
        return {
            'product.product': self.product_id.read(self.env['product.product']._get_fields_sn_wsd_barcode(), load=False),
            'uom.uom': self.product_id.uom_id.read(self.env['uom.uom']._get_fields_sn_wsd_barcode(), load=False),
            'stock.location': self.location_id.read(self.env['stock.location']._get_fields_sn_wsd_barcode(), load=False),
            'stock.lot': self.lot_id.read(self.env['stock.lot']._get_fields_sn_wsd_barcode(), load=False),
            'stock.package': self.package_id.read(self.env['stock.package']._get_fields_sn_wsd_barcode(), load=False),
            'res.partner': self.owner_id.read(self.env['res.partner']._get_fields_sn_wsd_barcode(), load=False),
        }
