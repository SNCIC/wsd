...
class ResConfigSettings(models.TransientModel):
    ...
    sn_wsd_barcode_demo_active = fields.Boolean("Demo Data Active", compute='_compute_sn_wsd_barcode_demo_active')
    show_barcode_nomenclature = fields.Boolean(compute='_compute_show_barcode_nomenclature')
    sn_wsd_barcode_mute_sound_notifications = fields.Boolean(
        "Mute Barcode application sounds",
        config_parameter="sn_wsd_barcode.mute_sound_notifications",
    )
    ...
    barcode_rfid_batch_time = fields.Integer(
        "RFID Timer", config_parameter="sn_wsd_barcode.barcode_rfid_batch_time"
    )
    barcode_separator_regex = fields.Char(
        "Multiscan Separator", config_parameter="sn_wsd_barcode.barcode_separator_regex",
    ...
    def _compute_show_barcode_nomenclature(self):
        ...
        for rec in self:
            rec.show_barcode_nomenclature = barcode_nomenclature_count > 1

    @api.depends('company_id')
    def _compute_sn_wsd_barcode_demo_active(self):
        for rec in self:
            rec.sn_wsd_barcode_demo_active = bool(self.env['ir.module.module'].search([('name', '=', 'sn_wsd_barcode'), ('demo', '=', True)]))
